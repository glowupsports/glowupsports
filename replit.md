...
- **Role-Specific Applications**: Dedicated applications for Coaches, Players, Platform Owners, and Service Providers. Service Provider app (`client/provider/`) has 3-tab layout (Today/Bookings/Profile) with booking management and status updates. Providers are created by academy owners via `POST /api/academy/shop/providers`; their role is `"service_provider"` in the users table. AppMode `"service_provider"` routes to `ProviderNavigator` in `RootStackNavigator`.
...
- **Group Social Hub**: Group-specific Events tab (social + match events with RSVP going/maybe/not_going), group Chat with emoji reactions and server-backed "seen" indicators. Tables: `group_events`, `group_event_rsvps`. Routes in `server/routes/social-features.ts`.
- **Session Waitlist**: Full group sessions show a join-waitlist option. Background scheduler promotes next player when a spot opens and sends claim-window notification (30 min default). Coach view shows waitlist in SessionDetailDrawer. Tables: `session_waitlist` with `offered_at` and `claim_window_minutes` columns.
- **Tournament Management**: Full tournament lifecycle — create, registration, draw generation (knockout/round-robin/group+knockout), manual draw adjustment, bracket view, result recording, XP award to winner. Push notifications for registration open, draw published, and result confirmed. Tables: `tournaments`, `tournament_participants`, `tournament_matches`. Routes in `server/routes/tournaments-ladders.ts`.
- **Ladder System**: Challenge-based player ladders with rank positions, wins/losses, and challenge lifecycle (pending/accepted/completed). Tables: `ladders`, `ladder_players`, `ladder_challenges`. Routes in `server/routes/tournaments-ladders.ts`.
- **Multiple Locations per Academy**: Academies can have multiple named locations with lat/lng. Sessions and series can be assigned to a specific location. Location filter chips appear in coach HQ and player schedule when multiple locations exist. Map links shown on session cards. Table: `locations`. Routes in `server/routes/admin-locations.ts`.
- **Live Scoring**: Real-time match scoring with public viewer access. Endpoints: `GET /player/me/active`, `GET /player/:id/active`. Live match banner on player profiles (own profile + public profile). CoachNavigator now includes `LiveMatchViewer` screen.
...
- **Player Calendar Integration**: Players can subscribe to their upcoming sessions as an ICS calendar feed (Google Calendar, Apple Calendar, or any ICS-compatible app). A calendar icon in the schedule header opens a modal with subscribe options. Each upcoming session card has a calendar icon to add that session directly to the native device calendar via `expo-calendar`. The ICS feed is at `GET /api/player/calendar/:playerId/sessions.ics?token=XXX` (HMAC-SHA256 authenticated). The token is fetched via `GET /api/player/me/calendar-token`.
...
- **Corporate/Business Accounts**: Companies purchase session credit pools for employees. DB tables: `corporate_accounts`, `corporate_members`, `corporate_credit_transactions`. `corporateStorage` (exported from `server/storage.ts`) handles all CRUD. API routes in `server/routes/corporate-accounts.ts`. Academy admins manage accounts via `AdminCorporateAccountsScreen` (accessible via Settings → Corporate Accounts). Employees accept invite tokens and view their benefits in `CorporateBenefitsScreen` (Player Settings → Corporate Benefits). Company contacts see a per-member usage dashboard at `CompanyContactDashboardScreen` (Player Settings → Company Dashboard). Invite emails sent via `sendCorporateEmployeeInviteEmail()`. **Booking integration**: `ensureCreditProcessed()` in `server/storage.ts` checks for active corporate membership (STEP 4b) before falling back to personal package credits; if a player has an active corporate account with sufficient credits, the cost is deducted from the corporate pool and recorded in `corporate_credit_transactions`.
