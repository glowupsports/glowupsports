...
- **Court Booking Confirmation Flow** (Task #1712): Coaches configure a `courtLocation` per coaching series (Overview tab inline edit → PATCH `/api/coach/series/:id`). Daily cron job (`server/courtBookingReminderJob.ts`) sends push reminders at 14/7/3 days before each session. Players tap the notification → `CourtBookingConfirmationScreen` → upload screenshot → POST `/api/player/sessions/:sessionId/court-booking-confirmation`. Coach sees per-session status in the new "Courts" tab of the Series Detail drawer (GET `/api/coach/series/:id/court-booking-confirmations`). Coach can approve/reject via PATCH `/api/coach/court-booking-confirmations/:id`. Screenshots stored in GCS `court-screenshots` bucket. Schema: `courtBookingConfirmations` table with status (`pending`/`confirmed`/`rejected`), `screenshotKey`, `rejectionNote`.
