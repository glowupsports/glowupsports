...
- **Glow Market & Community Marketplace**: Premium Nike-style tennis shop UI with hero banner, category pills, full-width service cards with Book buttons, product grid with Add to Bag buttons and discount badges, XP-based discounts, and Community Marketplace for used equipment. Shop prices in AED. Product cards feature images, descriptions, compare-at prices with discount percentages.
...
- **Credit System Architecture**: Player credits managed by `playerCreditPackages` table with specific types and expiry. Critical rules: Absent players are always charged. Single player in semi-private converts to `private_adjusted` and charges private credit. **Auto low-credit push notifications** sent to players/parents after session attendance when credits <= 2. **7-day credit expiry warnings** sent daily at 9 AM local academy time.
