...
- **V1 Orphan Debits — Write-off Decision (Task #901)**: Audit on 2026-04-21 confirmed the 14 rows surfaced by the watchdog are pure historical residue from before the V1 retirement work (Task #682 / #685). All 14 have `session_player_id IS NULL`, all live on `default-academy`, and all were written by code paths that have since been replaced by no-ops (`convertPackageConsumptionToDebt`), commented-out V1 bodies, or paths gated by the permanently-off `_v1WritesPermitted` flag. No active leak exists — every current writer of `session_consumed`/`session_debt` lives inside the V2-short-circuited `ensureCreditProcessed` path, which always populates `session_player_id`. Decision: **write them off in-place** by stamping `metadata.cancelled=true` (audit trail preserved — rows are NOT deleted). V2 academies derive every wallet/balance/debt read from `credit_ledger_v2`, so these legacy rows are inert and cancelling them changes no user-visible value. Run `npx tsx scripts/cancel-v1-orphan-debits.ts` for an audit, then `--apply` to silence the watchdog. The eventual full drop of `credit_transactions` is tracked separately (Task #676 phase 4–5).
...
- **V1 Read-Path Retirement (Task #958)**: Every user-facing credit read now hits a V2 source of truth (`player_credit_balance`, `credit_ledger_v2`, `credit_lots_v2`). Specifically: the Home dashboard (`/api/player/me/dashboard`), the admin player profile (`/api/admin/players/:playerId/stats` — both the `credits` summary and the `packages` list, which now derives from `credit_lots`), the player credit summary (`/api/players/:playerId/credits-summary`), and the parent packages view (`/api/parent/packages/:playerId`) all read via `storage.getPlayerCreditBalanceByType` and the `credit_lots` table. The legacy `packages.remainingCredits` field is no longer read in any user-facing path. Startup V1 repair scripts in `server/index.ts` were retired (`repairAllPlayerCredits`, `fixHolidayOvercharges`, `fixAlmaZaleskiCredits`, `fixRouzbehGhostCredit`, `auditAllPlayerCredits`, `reconcilePackageCredits`, the negative-`remaining_credits` clamp, and the one-time NULL `session_debt` cleanup) — non-credit maintenance (group session-type repair, ghost session cleanup, orphan `session_players`, session maintenance) was kept. The `[Reconcile] V1_ORPHAN_DEBITS` watchdog logging is unchanged and continues to surface stale historical V1 debits as a regression signal.
