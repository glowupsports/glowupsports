    ...
    try {
      ...
      const enrichedSeries = await Promise.all(
        allSeries.map(async (s) => {
          const seriesPlayersWithDetails =
            await storage.getSeriesPlayersWithDetails(s.id);
          const activePlayers = seriesPlayersWithDetails.filter(
            (p) => p.status === "active",
          );
          ...
          if (completedSessionIds.length > 0) {
            const feedbackCount = await db
              .select({
                count: sql<number>`count(distinct ${sessionFeedback.sessionId})`,
              })
              .from(sessionFeedback)
              .where(inArray(sessionFeedback.sessionId, completedSessionIds));
            pendingFeedback =
              sessionsForSeries.length - (feedbackCount[0]?.count || 0);
          }
      ...
      if (coachId && typeof coachId === "string") {
        ...
        console.log(
          `[AdminSeries] Coach ${coachId} orphan sessions found: ${orphanSessions.length}`,
        );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const creditBalances = await storage.getPlayersCreditBalances(
        playerIds,
        academyId,
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const {
        ...
        playerIds,
        sport,
        // Task #1033 — Open By Default for new lessons.
        isPublic,
        publicDropInPrice,
      } = req.body;
      ...
      if (!coachId) {
        return res
          .status(400)
          .json({ error: "coachId is required for admin series creation" });
      }
      ...
      const activeLocationCount = allLocations.filter(
        (l) => l.isActive !== false,
      ).length;
        ...
        if (!resolvedLocationId) {
          if (courtId) {
            ...
            if (!assignedCourt?.locationId) {
              return res.status(400).json({
                error:
                  "The selected court must be assigned to a location when the academy has multiple locations",
              });
            }
            ...
            return res.status(400).json({
              error:
                "Location is required when the academy has multiple locations",
            });
          }
        }
      }
      ...
      const series = await storage.createCoachingSeries({
        ...
        // Task #1033 — Open By Default. Privates are always closed.
        isPublic:
          sessionType === "private"
            ? false
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const academyId =
        req.user?.currentAcademyId || req.header("X-Academy-Id");
      ...
      if (id.startsWith("virtual-")) {
        ...
        console.log(
          `[Series DELETE] Deleting virtual series: ${id}, original key: ${originalSeriesKey}`,
        );

        // First, find the ORIGINAL series to know the original coach
        const originalSeries =
          originalSeriesKey !== "standalone"
        ...
        let orphanSessions;
        if (originalCoachId) {
          orphanSessions = allMatchingSessions.filter(
            (s) => s.coachId !== originalCoachId,
          );
          console.log(
            `[Series DELETE] Original coach: ${originalCoachId}, total sessions: ${allMatchingSessions.length}, orphan/transferred: ${orphanSessions.length}`,
          );
        } else {
          orphanSessions = allMatchingSessions;
          console.log(
            `[Series DELETE] No original series found, targeting all ${allMatchingSessions.length} sessions`,
          );
        }
        ...
        if (orphanSessions.length === 0) {
          return res.status(404).json({
            error:
              "No transferred/orphan sessions found for this virtual series",
          });
        }
        ...
        await db.transaction(async (tx) => {
          await tx
            ...
            .where(inArray(xpTransactions.sessionId, sessionIds));
          await tx
            .delete(coachXpTransactions)
            .where(inArray(coachXpTransactions.sessionId, sessionIds));
        ...
        console.log(
          `[Series DELETE] Deleted ${sessionIds.length} orphan sessions for virtual series ${id}`,
        );
      ...
      if (seriesCoachId) {
        ...
        console.log(
          "[Series DELETE] Cache invalidated for coach:",
          seriesCoachId,
        );
      }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!parsedAddPlayer.success)
        return res
          .status(400)
          .json({ error: fromZodError(parsedAddPlayer.error).message });
      ...
      if (existingPlayer && existingPlayer.status === "active") {
        return res
          .status(400)
          .json({ error: "Player is already in this series" });
      }
...
router.delete(
  "/api/admin/series/:id/players/:playerId",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
...
router.post(
  "/api/admin/series/:id/players/:playerId/pause",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
...
router.post(
  "/api/admin/series/:id/players/:playerId/unpause",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const isGroupSession =
        ...
        session.sessionType === "group_adjusted";
        ...
        if (isChargeable && !skipAbsentSemi && updated) {
          try {
            ...
            if (creditResult.action === "consumed") {
              console.log(
                `[AdminAttendance] Consumed credit for player ${record.playerId} in session ${id}`,
              );
            } else if (creditResult.action === "debt_created") {
              console.log(
                `[AdminAttendance] Created debt for player ${record.playerId} in session ${id} (no package)`,
              );
            }
          } catch (creditErr) {
            console.error(
              `[AdminAttendance] Credit processing failed for player ${record.playerId}:`,
              creditErr,
            );
          }
        }
      ...
      const allMarked = allSessionPlayers.every(
        (sp) => sp.attendanceStatus && sp.attendanceStatus !== "pending",
      );
      ...
      if (allMarked && session.status === "scheduled") {
        ...
        const presentPlayers = attendance.filter(
          (a: { status: string }) =>
        ...
        const eligiblePlayerIds = allSessionPlayers
          .filter(
            (sp) =>
              sp.attendanceStatus === "present" ||
              sp.attendanceStatus === "late",
          )
          .map((sp) => sp.playerId)
        ...
        if (eligiblePlayerIds.length > 0) {
          sendReflectionReminderForSession(id, eligiblePlayerIds).catch((err) =>
            console.error(
              `[ReflectionReminder] Error sending reminders for session ${id}:`,
              err,
            ),
          );
        }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!coachId || !courtId || !startTime || !duration || !sessionType) {
        return res.status(400).json({
          error:
            "Missing required fields: coachId, courtId, startTime, duration, sessionType",
        });
      }
      ...
      const sessionsToCreate =
        isRecurring && weekCount && weekCount > 1 ? weekCount : 1;
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Missing required fields: coachId, courtId, and flexibleSessions are required",
        });
      }
      ...
      if (playerIds && playerIds.length > 0) {
        ...
        console.log(
          "[BulkSessions DEBUG] - sessionType from request:",
          sessionType,
        );
        console.log("[BulkSessions DEBUG] - playerIds:", playerIds);
        console.log(
          "[BulkSessions DEBUG] - Player series found:",
          playerSeries.length,
        );
        playerSeries.forEach((s: any) => {
          console.log(
            "[BulkSessions DEBUG]   Series:",
            s.id,
            "title:",
            s.title,
            "coachId:",
          ...
          console.log(
            "[BulkSessions DEBUG]   Match check: coachId match=",
            s.coachId === coachId,
            "sessionType match=",
        ...
        if (existingSeries) {
          ...
          console.log(
            "[BulkSessions DEBUG] FOUND existing series:",
            existingSeries.id,
            existingSeries.title,
          );
        } else {
          console.log(
            "[BulkSessions DEBUG] NO existing series found - will create new one",
          );
        }
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const recentSessions = allSessions.filter(
        (s: any) =>
          new Date(s.startTime) >= thirtyDaysAgo && s.status !== "cancelled",
      );
    ...
    try {
      ...
      const upcomingSessions = sessionsThisWeek
        ...
        .map((s: any) => ({
          ...
          coachId: s.coachId,
          coachName:
            coaches.find((c: any) => c.id === s.coachId)?.name || "Unassigned",
          status: s.status,
        }));
...
router.get(
  "/api/admin/coaches/:coachId/stats",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const feedbackCount = await storage.getFeedbackCountByCoach(
        coachId,
        thirtyDaysAgo,
        now,
      );
...
router.post(
  "/api/admin/coaches/:coachId/payouts/:month/:year/pay",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      let payout = await storage.getCoachPayoutByMonthYear(
        coachId,
        parseInt(month),
        parseInt(year),
      );
...
router.post(
  "/api/admin/coaches/:coachId/payouts/:month/:year/decline",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedPayout = await storage.declineCoachPayout(
        payout.id,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const revenue = await storage.getAdminRevenueByMonth(
        academyId,
        year,
        month,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const creditBalance =
        await storage.getPlayerCreditBalanceByType(playerId);
      ...
      const totalCredits =
        computedCredits.group +
        computedCredits.semi_private +
        computedCredits.private;
      const playerLotsRes = await db.execute(sql`
        ...
        ORDER BY
          ...
          created_at DESC
      `);
      const playerLots = playerLotsRes.rows as {
        id: string;
        type: string;
      ...
      const activeLotCount = playerLots.filter(
      ...
      const allInvoices = playerAcademyId
        ? await storage.getInvoices(playerAcademyId)
        : [];
      const playerInvoices = allInvoices.filter(
        (inv: any) => inv.playerId === playerId,
      );
      const pendingInvoices = playerInvoices.filter(
        (inv: any) => inv.status === "pending" || inv.status === "sent",
      );
      const paidInvoices = playerInvoices.filter(
        (inv: any) => inv.status === "paid",
      );
      const invoiceTotalOwed = pendingInvoices.reduce(
      ...
      if (totalOwed > 0) {
        const hasOverdueInvoice = pendingInvoices.some(
          (inv: any) => inv.dueDate && new Date(inv.dueDate) < new Date(),
        );
      ...
      res.json({
        ...
        sessions: await (async () => {
          ...
          const sliceSessionIds = slice
            .map((s) => s.id)
            .filter(Boolean) as string[];
          const sliceSeriesIds = Array.from(
            new Set(slice.map((s) => s.seriesId).filter(Boolean)),
          ) as string[];
          ...
          return slice.map((s: any) => {
            ...
            const seriesName = s.seriesId
              ? (seriesNameById.get(s.seriesId) ?? null)
              : null;
            return {
              id: s.id,
...
router.post(
  "/api/admin/players/:playerId/fix-unpaid-sessions",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner", "coach"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const unpaidSessions = await db
        ...
        .where(
          and(
            ...
            or(eq(sessionPlayers.attendanceStatus, "present")),
...
router.get(
  "/api/admin/players/:playerId/credit-transactions",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner", "coach"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const transactions =
        await storage.getCreditTransactionsByPlayer(playerId);
      const creditBalance =
        await storage.getPlayerCreditBalanceByType(playerId);
      const playerPackages = await storage.getPlayerPackages(
        playerId,
      ...
      const summary = {
        ...
        semi_private: {
          credits: 0,
          debts: 0,
          balance: creditBalance.semi_private,
        },
...
router.post(
  "/api/admin/players/:playerId/add-debt",
  authMiddleware,
  requireRole("admin", "academy_owner", "platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!parsedDebt.success)
        return res
          .status(400)
          .json({ error: fromZodError(parsedDebt.error).message });
      ...
      if (
        !creditType ||
        !["group", "semi_private", "private"].includes(creditType)
      ) {
        return res.status(400).json({
          error: "Credit type must be group, semi_private, or private",
        });
      }
      ...
      return res.status(409).json({
        error:
          "This academy is on the new credit engine (V2). Manual V1 debt entries are no longer supported — adjust the player's credit balance directly instead.",
      });
    } catch (error) {
  ...
  async (_req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      for (const player of allPlayers) {
        ...
        const transactions = await storage.getCreditTransactionsByPlayer(
          player.id,
        );
        const creditBalance = await storage.getPlayerCreditBalanceByType(
          player.id,
        );

        const txByType = {
          group: { credits: 0, debts: 0 },
          semi_private: { credits: 0, debts: 0 },
          private: { credits: 0, debts: 0 },
        };
        ...
        const totalCreditsFromPackages =
          txByType.group.credits +
          txByType.semi_private.credits +
          txByType.private.credits;
        const netBalance =
          creditBalance.group +
          creditBalance.semi_private +
          creditBalance.private;

        if (
          totalSessions > 0 &&
          totalCreditsFromPackages === 0 &&
          netBalance >= 0
        ) {
          mismatches.push({
            playerId: player.id,
            ...
            byType: {
              group: {
                sessions: sessionsByType.group,
                balance: creditBalance.group,
                ...txByType.group,
              },
              semi_private: {
                sessions: sessionsByType.semi_private,
                balance: creditBalance.semi_private,
                ...txByType.semi_private,
              },
              private: {
                sessions: sessionsByType.private,
                balance: creditBalance.private,
                ...txByType.private,
              },
            },
          });
      ...
      console.log(
        `[CreditAudit] Found ${mismatches.length} players with potential credit mismatches`,
      );
  ...
  async (_req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const academyList = allAcademies.map((a) => {
        const academyPlayers = allPlayers.filter(
          (p) => p.academyId === a.id,
        ).length;
        const academyCoaches = allCoaches.filter(
          (c) => c.academyId === a.id,
        ).length;
        return {
          id: a.id,
...
router.post(
  "/api/platform/academies/:id/invites",
  authMiddleware,
  requireRole("platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!validRoles.includes(role)) {
        return res
          .status(400)
          .json({ error: "Invalid role. Must be 'academy_owner' or 'coach'" });
      }
...
router.post(
  "/api/platform/academies/:id/members",
  authMiddleware,
  requireRole("platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!role || !["academy_owner", "coach", "assistant"].includes(role)) {
        return res.status(400).json({
          error: "Valid role (academy_owner, coach, assistant) is required",
        });
      }
      ...
      const existingMembership = await storage.getCoachAcademyMembership(
        coachId,
        academyId,
      );
...
router.post(
  "/api/platform/academies/:id/users",
  authMiddleware,
  requireRole("platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        !role ||
        !["academy_owner", "coach", "assistant", "player"].includes(role)
      ) {
...
router.post(
  "/api/platform/impersonate/:academyId",
  authMiddleware,
  requireRole("platform_owner"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const ownerUsers = await db
        ...
        .from(users)
        .where(
          and(eq(users.role, "academy_owner"), eq(users.academyId, academyId)),
        )
        .limit(1);
      ...
      console.log(
        `[Impersonation] Platform owner ${req.user!.userId} impersonating academy ${academyId} (${academy.name})`,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const now = dateParam ? new Date(dateParam) : new Date();
      const upcomingSessions = await storage.getPlayerSessionsWithDetails(
        playerId,
        threeHoursAgo,
        future,
      );
      ...
      // featured `nextSession`; the rest power the row list under the hero.
      {
        ...
        await Promise.all([
          ...courtIds.map(async (id) => {
            courtMap.set(id, await storage.getCourt(id));
          }),
          ...coachIds.map(async (id) => {
            coachMap.set(id, await storage.getCoach(id));
          }),
        ]);
      ...
      if (sortedSessions.length > 0) {
        ...
        const court = session.courtId
          ? await storage.getCourt(session.courtId)
          : null;
        ...
        const durationMinutes =
          session.startTime && session.endTime
            ...
            : null;
        const sessionPlayerRecord = await storage.getSessionPlayer(
          session.id,
          playerId,
        );
      ...
      const pastSessions = await storage.getPlayerSessionsWithDetails(
        playerId,
        thirtyDaysAgo,
        now,
      );
      const attendedSessions = pastSessions.filter(
      ...
      const totalCredits =
        creditsByType.group +
        creditsByType.private +
        creditsByType.semi_private;
      ...
      try {
        ...
        if (active) {
          const reqCoach = active.coachId
            ? await storage.getCoach(active.coachId)
            : null;
          pendingRequest = {
            id: active.id,
            status: active.status as
              | "pending"
        ...
        console.error(
          "[Dashboard] Failed to load pending booking request (non-fatal):",
          pendingReqErr,
        );
      }
    ...
    try {
      ...
      const sessionsWithDetails = await Promise.all(
        groupSessions.map(async (session) => {
          ...
          const isEnrolled = sessionPlayers.some(
          ...
          return {
            ...
            ballLevel:
              (
                (session as any).targetBallLevel ||
                (session as any).ballLevel ||
                ""
              ).toUpperCase() || null,
            isEnrolled,
            locationName,
...
router.post(
  "/api/player/sessions/:sessionId/enroll",
  authMiddleware,
  requirePlayerOrOwner,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (new Date(session.startTime) <= new Date()) {
        return res
          .status(400)
          .json({ error: "Cannot enroll in past sessions" });
      }
      ...
      if (credits.group <= 0) {
        return res.status(400).json({
          error: "No group credits available. Please purchase a package.",
        });
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const acceptedConnections = await db
        ...
        .where(
          and(
            ...
            or(eq(pc.player1Id, playerId), eq(pc.player2Id, playerId)),
          ),
        );
      ...
      const pendingReceived = await db
        .select()
        .from(pc)
        .where(and(eq(pc.status, "pending"), eq(pc.player2Id, playerId)));
      ...
      const friends = (
        await Promise.all(friendPlayerIds.map(fetchPlayerInfo))
      ).filter(Boolean);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (player.academyId) {
        const academyPlayers = await storage.getPlayersByAcademy(
          player.academyId,
        );
        ...
        const activatedPlayerIds = new Set(
          activatedUsers.map((u) => u.playerId).filter(Boolean) as string[],
        );
        ...
        const playersWithDistance = sameLevelPlayers.map((p) => {
          ...
          if (myLat != null && myLon != null && pLat != null && pLon != null) {
            distanceKm =
              Math.round(haversineKm(myLat, myLon, pLat, pLon) * 10) / 10;
          }
        ...
        for (const { player: p, distanceKm } of playersWithDistance.slice(
          0,
          20,
        )) {
          nearbyPlayers.push({
      ...
      const sportFilterParam = (
        req.query.sport as string | undefined
      )?.toLowerCase();
      const typeFilterParam = (
      ...
      const levelFallbackParam = (
        req.query.levelFallback as string | undefined
      )?.toLowerCase();
      ...
      if (player.academyId) {
          ...
          try {
            ...
            for (const series of seriesRows) {
              if ((series as any)?.ballLevel) {
                seriesLevelMap.set(
                  series.id,
                  ((series as any).ballLevel as string).toLowerCase(),
                );
              }
              if ((series as any)?.sessionPrice) {
                seriesPriceMap.set(
                  series.id,
                  Number((series as any).sessionPrice),
                );
              }
        ...
        const filterByAllowedLevels = (allowedLevels: Set<string>) =>
          academySessions.filter((s) => {
            ...
            const seriesLevel = (s as any).seriesId
              ? seriesLevelMap.get((s as any).seriesId) || ""
              : "";
            ...
            if (sportFilterParam && sportFilterParam !== "all") {
              const rawSessionSport: string =
                ((s as any).sport as string | undefined) ?? "";
              const rawSeriesSport: string = (s as any).seriesId
                ? (seriesSportMap.get((s as any).seriesId) ?? "")
                : "";
              const effectiveSport = (
                rawSessionSport || rawSeriesSport
              ).toLowerCase();
              if (!effectiveSport || effectiveSport !== sportFilterParam)
                return false;
            }
        ...
        let levelFilteredSessions = filterByAllowedLevels(
          new Set([playerBallLevel]),
        );
        ...
        if (levelFallbackParam === "adjacent") {
          ...
          if (myLevelGroupCount === 0) {
            const idx = (BALL_LEVEL_ORDER as readonly string[]).indexOf(
              playerBallLevel,
            );
            if (idx >= 0) {
              const adjacent = [
        ...
        const sortedSessions = levelFilteredSessions.sort(
          (a, b) =>
            new Date(a.startTime).getTime() - new Date(b.startTime).getTime(),
        );
        totalSessionCount = sortedSessions.length;
        const upcomingSessions = sortedSessions.slice(
          offsetParam,
        ...
        const _coachRows = _coachIds.length
          ? await db
              .select()
              .from(coaches)
              .where(inArray(coaches.id, _coachIds))
          : [];
        ...
        const _seriesIdsNeedingPlayers = [
          ...new Set(
            upcomingSessions
              .filter(
                (s) =>
                  (s as any).seriesId &&
                  (_spBySession.get(s.id)?.length ?? 0) === 0,
              )
        ...
        for (const s of upcomingSessions) {
          const ids =
            _spBySession.get(s.id) && _spBySession.get(s.id)!.length > 0
              ? _spBySession.get(s.id)!
              : (s as any).seriesId
                ? _seriesPlayerMap.get((s as any).seriesId) || []
                : [];
        ...
        const _playerRows = _allPlayerIds.size
          ? await db
              .select()
              .from(players)
              .where(inArray(players.id, [..._allPlayerIds]))
          : [];
        ...
        const _locationRows = _locationIds.length
          ? await db
              .select()
              .from(locations)
              .where(inArray(locations.id, _locationIds))
          : [];
        ...
        for (const session of upcomingSessions) {
          const coach = session.coachId
            ? _coachMap.get(session.coachId) || null
            : null;
          ...
          if (
            ...
            sessionDistanceKm =
              Math.round(
                haversineKm(myLat, myLon, locationLat, locationLng) * 10,
              ) / 10;
          }

          const effectiveBallLevel = (
            (session as any).ballLevel ||
            ((session as any).seriesId
              ? seriesLevelMap.get((session as any).seriesId)
              : "") ||
            ""
          ).toLowerCase();
          const rawSessionPrice: number | undefined =
            (session as any).price != null
              ? Number((session as any).price)
              : undefined;
          const rawSeriesPrice: number | undefined = (session as any).seriesId
            ? seriesPriceMap.get((session as any).seriesId)
            : undefined;
          const sessionPrice: number | undefined =
            rawSessionPrice ?? rawSeriesPrice;
          const rawSessionSportVal: string | undefined = (session as any)
            .sport as string | undefined;
          const rawSeriesSportVal: string | undefined = (session as any)
            .seriesId
            ? seriesSportMap.get((session as any).seriesId)
            : undefined;
      ...
      if (player.academyId && includeOpenMatches) {
        ...
        for (const match of openMatchRequests) {
          ...
          const dateOnly =
            match.preferredDate || new Date().toISOString().split("T")[0];
          ...
          if (
            match.preferredTime &&
            /^\d{1,2}:\d{2}/.test(match.preferredTime)
          ) {
            const [hh, mm] = match.preferredTime
              .split(":")
              .map((v) => parseInt(v, 10));
      ...
      const skillTrends: { skill: string; trend: string; label: string }[] = [
      ...
      if (player.academyId) {
        ...
        groupSessions = upcomingAll.filter(
          (s) => s.sessionType === "group",
        ).length;
        privateLessons = upcomingAll.filter(
          (s) => s.sessionType === "private",
        ).length;
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await db.execute(
        sql`UPDATE players SET last_latitude = ${latitude}, last_longitude = ${longitude}, location_updated_at = NOW() WHERE id = ${playerId}`,
      );
      res.json({ success: true });
    } catch (error) {
...
const newsCaches: Record<
  string,
  { articles: any[]; fetchedAt: number } | null
> = {
  tennis: null,
  padel: null,
  pickleball: null,
...
const SPORT_FEEDS: Record<string, { url: string; source: string }[]> = {
  tennis: [
    {
  ...
  padel: [
    {
      url: "https://www.worldpadeltour.com/en/rss/",
  ...
  pickleball: [
    {
      url: "https://thepickler.com/blogs/pickleball-blog.atom",
      source: "The Pickler",
    },
    { url: "https://www.ppatour.com/feed/", source: "PPA Tour" },
    {
      url: "https://pickleballcentral.com/blogs/news.atom",
...
const SPORT_FALLBACKS: {
  ...
  {
    id: "tf1",
    title: "Australian Open 2026: Draw Announced",
    link: "#",
    source: "ATP Tour",
  ...
  {
    id: "tf2",
    title: "Sinner Leads ATP Rankings After Strong Start",
    link: "#",
    source: "ATP Tour",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf3",
    title: "Swiatek Eyes Fourth Grand Slam Title",
    link: "#",
    source: "WTA",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf4",
    title: "Alcaraz Working on New Serve Technique",
    link: "#",
    source: "ESPN",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf5",
    title: "Djokovic Confirms Melbourne Participation",
    link: "#",
    source: "BBC Sport",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf6",
    title: "Young Stars Rising in ATP Next Gen Finals",
    link: "#",
    source: "ATP Tour",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf7",
    title: "WTA Finals: Key Matchups to Watch",
    link: "#",
    source: "WTA",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf8",
    title: "Tennis Technology: New Hawkeye Updates",
    link: "#",
    source: "Tennis",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf9",
    title: "Roland Garros Clay Court Renovations Complete",
    link: "#",
    source: "Tennis",
    publishedAt: new Date().toISOString(),
  },
  {
    id: "tf10",
    title: "Doubles Specialists Dominate Miami Open",
    link: "#",
    source: "ATP Tour",
    publishedAt: new Date().toISOString(),
  },
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const sport =
        ...
        ["tennis", "padel", "pickleball"].includes(req.query.sport)
          ? req.query.sport
          : "tennis";
      ...
      for (const feed of feeds) {
        try {
          ...
          for (const item of feedItems) {
            ...
            articles.push({
              id:
                item.guid ||
                item.link ||
                `${feed.source}-${Date.now()}-${Math.random()}`,
              title,
              link: item.link || "",
              source: feed.source,
          ...
          console.log(
            `[News] Failed to fetch ${feed.source}:`,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const lessonsTaughtRow = await db
        .select({ count: sql<number>`count(*)` })
        .from(sessions)
        .where(
          and(eq(sessions.coachId, coachId), eq(sessions.status, "completed")),
        );
      ...
      const now = new Date();
      const upcomingPublicSessions = await db
        .select({
          id: sessions.id,
      ...
      const upcomingSessionsEnriched = await Promise.all(
        upcomingPublicSessions.map(async (s) => {
      ...
      const reviewPlayerIds = recentReviewRows
        .map((r) => r.playerId)
        .filter(Boolean) as string[];
      const reviewPlayers =
        reviewPlayerIds.length > 0
          ? await db
              .select({ id: players.id, name: players.name })
              .from(players)
              .where(inArray(players.id, reviewPlayerIds))
          : [];
      const playerNameMap = new Map(
        reviewPlayers.map((p) => [p.id, p.name?.split(" ")[0] || "Player"]),
      );

      const reviewsFormatted = recentReviewRows.map((r) => ({
        id: r.id,
        overallScore: r.overallScore
          ? parseFloat(r.overallScore.toString())
          : null,
      ...
      res.json({
        ...
        totalLessonsTaught,
        averageRating: reviewStats?.averageOverall
          ? parseFloat(reviewStats.averageOverall.toString())
          : null,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const coachIds = [
        ...new Set(
          sessions.map((s) => s.coachId).filter((id): id is string => !!id),
        ),
      ];
      const courtIds = [
        ...new Set(
          sessions.map((s) => s.courtId).filter((id): id is string => !!id),
        ),
      ];
      const directLocationIds = [
        ...new Set(
          sessions.map((s) => s.locationId).filter((id): id is string => !!id),
        ),
      ];
      ...
      const playerSessions = activeSessions.map((session) => {
        ...
        return {
          ...
          session: {
            ...
            courtName: session.courtId
              ? courtMap[session.courtId] || null
              : null,
            title: getSessionTitle(displaySessionType),
            locationId: (() => {
              const fromCourt = session.courtId
                ? courtToLocationId[session.courtId]
                : undefined;
              return fromCourt ?? session.locationId ?? null;
            })(),
            locationName: (() => {
              const fromCourt = session.courtId
                ? courtLocationMap[session.courtId]?.name
                : undefined;
              const fromDirect = session.locationId
                ? locationInfoMap[session.locationId]?.name
                : undefined;
              return fromCourt ?? fromDirect ?? null;
            })(),
            locationAddress: (() => {
              const fromCourt = session.courtId
                ? courtLocationMap[session.courtId]?.address
                : undefined;
              const fromDirect = session.locationId
                ? locationInfoMap[session.locationId]?.address
                : undefined;
              return fromCourt ?? fromDirect ?? null;
            })(),
            locationLat: (() => {
              const fromCourt = session.courtId
                ? courtLocationMap[session.courtId]?.lat
                : undefined;
              const fromDirect = session.locationId
                ? locationInfoMap[session.locationId]?.lat
                : undefined;
              return fromCourt ?? fromDirect ?? null;
            })(),
            locationLng: (() => {
              const fromCourt = session.courtId
                ? courtLocationMap[session.courtId]?.lng
                : undefined;
              const fromDirect = session.locationId
                ? locationInfoMap[session.locationId]?.lng
                : undefined;
              return fromCourt ?? fromDirect ?? null;
            })(),
          },
          ...
          ballLevel: session.ballLevel ?? null,
...
router.get(
  "/api/player/me/court-bookings",
  authMiddleware,
  requirePlayerOrOwner,
  requireFeatureUnlock("court_booking"),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
