...
export const storage = {
  ...
  async deleteAcademy(id: string): Promise<void> {
    ...
    await db.transaction(async (tx) => {
      ...
      // matchOpponents has no matchId column — it's keyed by playerId.
      // Null out matches.opponentId first (nullable FK) so we can delete matchOpponents rows
      // before deleting matches in Phase 12b.
      if (matchIds.length > 0) {
        await tx.update(matches).set({ opponentId: null }).where(inArray(matches.id, matchIds));
      ...
      // Null out matchPlans.opponentId before deleting matchOpponents
      // (matchPlans.opponentId → matchOpponents.id is a nullable FK)
      if (playerIds.length > 0) {
        await tx.update(matchPlans).set({ opponentId: null }).where(inArray(matchPlans.playerId, playerIds));
      }
      // Delete matchOpponents by playerId (the actual FK column on that table)
      if (playerIds.length > 0) {
        await tx.delete(matchOpponents).where(inArray(matchOpponents.playerId, playerIds));
      }
      ...
      // Delete coachTimeBlocks for this academy's coaches (coachId FK is NOT NULL, non-cascading)
      // and nullify sourceAcademyId on any remaining blocks from other academies that referenced this academy.
      if (coachIds.length > 0) {
        await tx.delete(coachTimeBlocks).where(inArray(coachTimeBlocks.coachId, coachIds));
      }
      ...
      if (playerIds.length > 0) {
        ...
        // Tables added after original deleteAcademy was written — these all have
        // non-cascading FKs to players.id and must be deleted before players.
        await tx.delete(playerBadges).where(inArray(playerBadges.playerId, playerIds));
        await tx.delete(playerTitles).where(inArray(playerTitles.playerId, playerIds));
        await tx.delete(playerQuests).where(inArray(playerQuests.playerId, playerIds));
        await tx.delete(dailyQuestSlots).where(inArray(dailyQuestSlots.playerId, playerIds));
        await tx.delete(playerStreaks).where(inArray(playerStreaks.playerId, playerIds));
        await tx.delete(playerSeasonEnrollments).where(inArray(playerSeasonEnrollments.playerId, playerIds));
        await tx.delete(weeklyDigests).where(inArray(weeklyDigests.playerId, playerIds));
        await tx.delete(monthlyDigests).where(inArray(monthlyDigests.playerId, playerIds));
        await tx.delete(yearlyRecaps).where(inArray(yearlyRecaps.playerId, playerIds));
      ...
      // drillBlocks.templateId → lessonTemplates.id (NOT NULL, non-cascading) — must go before lessonTemplates
      // Collect lesson template IDs for this academy to delete their drill blocks
      const academyLessonTemplateRecords = await tx.select({ id: lessonTemplates.id }).from(lessonTemplates).where(eq(lessonTemplates.academyId, id));
      const lessonTemplateIds = academyLessonTemplateRecords.map(t => t.id);
      if (lessonTemplateIds.length > 0) {
        await tx.delete(drillBlocks).where(inArray(drillBlocks.templateId, lessonTemplateIds));
      }
      await tx.delete(lessonTemplates).where(eq(lessonTemplates.academyId, id));
  ...
  async deletePlayer(id: string, academyId: string | null): Promise<boolean> {
    ...
    try {
      ...
      // ---------------------------------------------------------------
      // Task #904 / #907: player_* / player-referencing tables whose
      // cleanup is a straight "DELETE FROM <table> WHERE player_id" (or
      // small variants thereof). The canonical ordered list lives in
      // `server/lib/player-cleanup.ts` and is also consumed by the
      // startup drift watchdog (`audit-player-fks.ts`) so adding a new
      // player_* table in one place picks it up everywhere.
      //
      // Each delete is existence-guarded (to_regclass) so older databases
      // that haven't run db:push for the latest schema migrations stay
      // safe (no-op). The merge endpoint in `server/routes/admin-setup.ts`
      // either transfers these rows (Part A) or deletes them (Part B);
      // see `GUARDED_PLAYER_DELETE_STATEMENTS` for per-entry merge notes.
      // ---------------------------------------------------------------
      // Task #906's credit_shadow_diff + slot_reservations are folded into
      // `GUARDED_PLAYER_DELETE_STATEMENTS` in the shared module below.
      await runGuardedPlayerDeletes(
      ...
      await Promise.all([
        ...
        // yearly_recaps references feed_items via feed_item_id FK — delete before feed_items
        db.delete(yearlyRecaps).where(eq(yearlyRecaps.playerId, id)),
        // Community feed posts authored by this player
        db.delete(feedItems).where(eq(feedItems.authorPlayerId, id)),
      ]);
