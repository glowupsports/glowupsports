...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  app.delete("/api/cleanup/series/:id", async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message: `Series ${id} and all related sessions deleted`,
      });
    } catch (error) {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        for (const player of allPlayers) {
          ...
          const vacationCount = attendanceData.filter(
            (a) => a.status === "vacation",
          ).length;
          ...
          for (const [sessionId, debits] of debitsBySession) {
            ...
            for (const dup of toDelete) {
              await db
                ...
                .where(eq(creditTransactions.id, dup.id));
              duplicatesRemoved++;
              totalDuplicatesRemoved++;
            }
          }

          const afterDebits = await db
            .select({
              amount: creditTransactions.amount,
              creditType: creditTransactions.creditType,
            })
      ...
      try {
        ...
        `);
          ...
          for (const sp of spRecords.rows as any[]) {
            ...
            const isPrivate =
              sessionType === "private" || sessionType === "private_adjusted";
          ...
          for (const orphan of orphanDebits) {
            await db
              .delete(creditTransactions)
              .where(eq(creditTransactions.id, orphan.id));
            removedCount++;
          }
          ...
          for (const [sessionId, debitsForSession] of debitsBySession) {
            ...
            for (let i = 1; i < sorted.length; i++) {
              await db
                .delete(creditTransactions)
                .where(eq(creditTransactions.id, sorted[i].id));
              removedCount++;
            }
          }
  ...
  app.post(
    "/api/debug/reset-orphaned-session-players",
    async (req: Request, res: Response) => {
      try {
      ...
      `);
        ...
        console.log(
          `[FillMissing] Found ${missingDebits.rows.length} chargeable session_players without matching debit`,
        );
  ...
  app.post(
    "/api/fix-player-unpaid/:playerId",
    async (req: Request, res: Response) => {
      try {
        ...
        const allAttendedSessions = await db
          .select({
          ...
          try {
            ...
            await db
              ...
              .set({
                ...
                creditTransactionId: debtId,
              })
              .where(eq(sessionPlayers.id, session.sessionPlayerId));

            fixedCount++;
            console.log(
              `[FixUnpaid] Recorded debt for session ${session.sessionId} (type: ${creditType})`,
            );
          } catch (e: any) {
            if (e.code === "23505") {
              console.log(
                `[FixUnpaid] Transaction already exists for ${session.sessionId}, skipping`,
              );
            } else {
              console.error(
                `[FixUnpaid] Error fixing session ${session.sessionId}:`,
                e,
              );
            }
          }
        ...
        for (const [playerId, playerSessions] of byPlayer) {
          ...
          for (const session of playerSessions) {
            ...
            try {
              ...
              totalFixed++;
              console.log(
                `[FixAllUnpaid] Recorded debt for session ${session.sessionId} (type: ${creditType})`,
              );
            } catch (e: any) {
              if (e.code === "23505") {
                console.log(
                  `[FixAllUnpaid] Transaction already exists for ${session.sessionId}, skipping`,
                );
              } else {
                console.error(
                  `[FixAllUnpaid] Error fixing session ${session.sessionId}:`,
                  e,
                );
              }
            }
          ...
          if (fixedCount > 0) {
            ...
            console.log(
              `[FixAllUnpaid] Fixed ${fixedCount} sessions for ${player?.name || playerId}`,
            );
          }
        }
