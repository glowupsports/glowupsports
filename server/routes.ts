...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/fix-all-unpaid-sessions", async (_req: Request, res: Response) => {
    try {
      ...
      const playersFixes: Array<{playerId: string, playerName: string, fixed: number}> = [];
      let totalFixed = 0;
      ...
      for (const [playerId, playerSessions] of byPlayer) {
        ...
        if (fixedCount > 0) {
          ...
          console.log(`[FixAllUnpaid] Fixed ${fixedCount} sessions for ${player?.name || playerId}`);
        }
      }
  ...
  app.get("/api/players/:id/attendance-report", authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (allSessionIds.length > 0) {
        const sessionTxs = await db.select({
          sessionId: creditTransactions.sessionId,
          reason: creditTransactions.reason,
          packageId: creditTransactions.packageId,
      ...
      const records = playerRecords
        .map(record => {
          ...
          const isPaid = record.sessionId && paidSessionIdSet.has(record.sessionId);
          ...
          return {
            ...
            paymentStatus: isCancelled ? "cancelled"
              : isNoCharge ? "no_charge"
              : isPaid ? "paid"
              : "pending",
          };
        })
  ...
  app.post("/api/player/me/onboarding", authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { academyId, motivationType, dateOfBirth, ballLevel, height, tshirtSize, dominantHand, backhandType, experienceLevel, enjoymentTags, focusGoals, selfConfidenceFlags } = req.body;
