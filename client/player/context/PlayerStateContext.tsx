...
import { buildPhotoUrl, getApiUrl, apiRequest } from "@/lib/query-client";
...
export function PlayerStateProvider({ children }: { children: ReactNode }) {
  ...
  const state = useMemo((): PlayerState => {
    ...
    return {
      ...
      coachPhotoUrl: buildPhotoUrl(((nextSession as any)?.coachPhotoUrl as string | null | undefined) ?? dashboardData.coach?.photoUrl) || null,
