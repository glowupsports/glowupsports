...
A defensively discovered accepted decision with no remaining eligible contribution is terminal `NO_CHANGE`, not `SUPERSEDED`. It preserves the decision, evidence links, rationale, and an execution audit record while leaving state version, history, snapshots, and receipts unchanged.

**Why:** `SUPERSEDED` means another decision replaced it; it must not be a generic no-op label.